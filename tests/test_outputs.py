...
def _latest_projection_file():
    ...
    if not projection_dir.exists():
        pytest.skip("outputs/projections directory does not exist — run inference first")
    files = sorted(projection_dir.glob("*_projections.csv"))
    if not files:
        pytest.skip("No projection files found in outputs/projections — run inference first")
    return files[-1]
