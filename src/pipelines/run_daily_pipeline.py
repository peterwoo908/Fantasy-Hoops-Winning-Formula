...
from src.config import MODEL_READY_PATH, PROJECTIONS_DIR, TEAM_DATA_PATH
...
def run_daily_pipeline(date_str: str | None = None) -> pd.DataFrame:
    ...
    if not TEAM_DATA_PATH.exists():
        raise FileNotFoundError(f"Team data not found at {TEAM_DATA_PATH}. Run build-dataset first.")
